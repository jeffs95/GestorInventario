    ...
    <div class="card">
      ...
      <div class="card-body">
        <form method="POST" action="{{ route('usuarios.store') }}">
          ...
          <div class="row">
            <div class="col-md-6 mb-3">
              <label class="form-label text-xs fw-bold">Contraseña <span class="text-danger">*</span></label>
              <input type="password" name="password" class="form-control" required minlength="6">
            </div>
            <div class="col-md-6 mb-3">
              <label class="form-label text-xs fw-bold">Confirmar contraseña <span class="text-danger">*</span></label>
              ...
              <div class="border rounded p-2">
                ...
                <div class="form-check mb-1">
                  <input type="checkbox" name="roles[]" value="{{ $rol->id }}"
                         class="form-check-input" id="rol_new_{{ $rol->id }}"
                         {{ in_array($rol->id, old('roles', [])) ? 'checked' : '' }}>
                  <label class="form-check-label text-sm" for="rol_new_{{ $rol->id }}">
                    {{ $rol->nombre }}
                    @if($rol->descripcion)
